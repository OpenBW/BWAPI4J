...
//	BWTA
//
jobject createPolygon(JNIEnv *env, const BWTA::Polygon &polygon, jclass positionClass, jmethodID positionNew, jclass polygonClass, jmethodID polygonNew,
...
JNIEXPORT void JNICALL Java_bwta_BWTA_analyze(JNIEnv *env, jobject caller, jobject bwtaObject) {
  ...
  for (const BWTA::Region *region : BWTA::getRegions()) {
    ...
    env->SetObjectField(jRegion, env->GetFieldID(regionClass, "polygon", "Lbwta/Polygon;"),
                        createPolygon(env, region->getPolygon(), positionClass, positionNew, polygonClass, polygonNew, arrayListAdd));
  ...
  for (const BWTA::BaseLocation *baseLocation : BWTA::getBaseLocations()) {
    ...
    jobject jtilePosition =
        env->NewObject(tilePositionClass, tilePositionNew, (jint)baseLocation->getTilePosition().x, (jint)baseLocation->getTilePosition().y);
...
JNIEXPORT void JNICALL Java_bwta_BWTA_computeDistanceTransform(JNIEnv *, jobject) { BWTA::computeDistanceTransform(); }

JNIEXPORT void JNICALL Java_bwta_BWTA_balanceAnalysis(JNIEnv *, jobject) { BWTA::balanceAnalysis(); }

JNIEXPORT void JNICALL Java_bwta_BWTA_cleanMemory(JNIEnv *, jobject) { BWTA::cleanMemory(); }

JNIEXPORT jlong JNICALL Java_bwta_BWTA_getRegionT(JNIEnv *, jobject, jint x, jint y) { return (jlong)BWTA::getRegion(TilePosition(x, y)); }

JNIEXPORT jlong JNICALL Java_bwta_BWTA_getRegionP(JNIEnv *, jobject, jint x, jint y) { return (jlong)BWTA::getRegion(Position(x, y)); }
