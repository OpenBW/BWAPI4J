      ...
      try {
        ...
        if (bwem.getMap().getUnassignedStartingLocations().size() > 0) {
          throw new IllegalStateException(
              "Failed to find suitable bases for the following starting locations: "
                  + bwem.getMap().getUnassignedStartingLocations().toString());
        }
      }
      ...
      /* Highlight starting locations and possible base locations. */ {
        for (final Base base : bwem.getMap().getBases()) {
          ...
          if (isOnScreen(baseLocation)) {
            bw.getMapDrawer()
                .drawBoxMap(baseLocation, baseLocation.add(resourceDepotSize), highlightColor);
          }
