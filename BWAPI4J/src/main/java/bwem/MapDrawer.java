...
public class MapDrawer {
  ...
  public boolean processCommand(String command) {
    ...
    if (processCommandVariants(command, Command.SHOW_UNBUILDABLE.toString(), showUnbuildable))
      return true;
    if (processCommandVariants(command, Command.SHOW_GROUND_HEIGHT.toString(), showGroundHeight))
      return true;
    if (processCommandVariants(command, Command.SHOW_MINERALS.toString(), showMinerals))
      return true;
    if (processCommandVariants(command, Command.SHOW_GEYSERS.toString(), showGeysers)) return true;
    if (processCommandVariants(
        command, Command.SHOW_STATIC_BUILDINGS.toString(), showStaticBuildings)) return true;
    if (processCommandVariants(command, Command.SHOW_BASES.toString(), showBases)) return true;
    if (processCommandVariants(
        command, Command.SHOW_ASSIGNED_RESOURCES.toString(), showAssignedResources)) return true;
    if (processCommandVariants(command, Command.SHOW_FRONTIER.toString(), showFrontier))
      return true;
                  ...
                  if (processCommandVariants(command, "all", showBases))
                    if (processCommandVariants(command, "all", showAssignedResources))
                      if (processCommandVariants(command, "all", showFrontier))
                        if (processCommandVariants(command, "all", showCP)) return true;

    return false;
  }

  private boolean processCommandVariants(
      String command, String attributName, MutableBoolean attribut) {
