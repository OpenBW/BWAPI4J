...
public class MapPrinter {
  ...
  public final boolean showContinents =
      !showAreas; // renders continents with distinct colors, unless showAreas == true
  ...
  public final boolean showMinerals =
      showResources; // prints the minerals, unless showResources == false
  public final boolean showGeysers =
      showResources; // prints the geysers, unless showResources == false
  public final boolean showStaticBuildings = true; // prints the StaticBuildings
  public final boolean showBlockingBuildings =
      true; // renders the StaticBuildings and the minerals that are blocking some ChokePoint
  public final boolean showStackedNeutrals =
      true; // renders the StaticBuildings and the minerals that are stacked
  public final boolean showStartingLocations =
      true; // renders the bases that are at starting locations
  public final boolean showBases = true; // prints the bases suggested by BWEM
  public final boolean showAssignedResources =
      showBases; // renders the Resources assigned to each Base, unless showBases == false
  ...
  //
  ////	Uncomment the 2 lines below to write one more copy of the bitmap, in the folder of the map
  ...
  public void initialize(BW pBW, Map pMap) {
    ...
    image =
        new BufferedImage(
            pBW.getBWMap().mapWidth() * 4,
            pBW.getBWMap().mapHeight() * 4,
  ...
  public void rectangle(
    ...
    for (int y = topLeft.getY(); y <= bottomRight.getY(); ++y)
      for (int x = topLeft.getX(); x <= bottomRight.getX(); ++x)
        if ((fillMode == fill_t.fill)
            || (y == topLeft.getY())
            || (y == bottomRight.getY())
            || (x == topLeft.getX())
            || (x == bottomRight.getX()))
          if ((dashedMode == dashed_t.not_dashed) || ((x + y) & 1) != 0) point(x, y, col);
  }

  public void rectangle(
      WalkPosition topLeft, WalkPosition bottomRight, Color col, fill_t fillMode) {
    ...
    for (int y = center.getY() - radius; y <= center.getY() + radius; ++y)
      for (int x = center.getX() - radius; x <= center.getX() + radius; ++x)
        if ((fillMode == fill_t.fill)
            ...
            || (x == center.getX() + radius))
          if (map.getData().getMapData().isValid(new WalkPosition(x, y))) point(x, y, col);
  }
      ...
      for (int x = center.getX() - radius; x <= center.getX() + radius; ++x) {
        ...
        if (BwemExt.dist(w, center) <= radius)
          if ((fillMode == fill_t.fill) || (BwemExt.dist(w, center) >= radius - 1))
            if (map.getData().getMapData().isValid(w)) point(x, y, col);
      }
  }
