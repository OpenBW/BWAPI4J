...
public final class YATA {
    ...
    for (int y = 0; y < getHeight(); ++y) {
      for (int x = 0; x < getWidth(); ++x) {
        ...
        final WalkPosition deltas[] = {
          walkPosition,
          walkPosition.add(new WalkPosition(1, 0)),
          walkPosition.add(new WalkPosition(0, 1)),
          walkPosition.add(new WalkPosition(1, 1))
        };
  ...
  public List<TilePosition> getPath(final TilePosition start, final TilePosition end) {
    ...
    final List<GridCell> gridCellPath =
        this.finder.findPath(startNode, endNode, this.navigationGrid);
